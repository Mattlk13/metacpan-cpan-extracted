package UUID;
require 5.005063; # see MIN_PERL_VERSION in Makefile.PL
use strict;
use warnings;
use Carp 'croak';
...
Version 4 and 7 UUIDs are partially filled with random numbers, as well
...
Starting in version 0.33, UUID generates random numbers itself using the
...
Starting in version 0.38, the whole mix is initially seeded from the
best platform-specific source available which could be one or more of:
...
Time-based UUIDs are formed with time-of-day information gathered from
I<Time::HiRes>. This provides a portable timestamp with 1us resolution
facilitating generation rates up to 1M UUIDs/sec without further action.

If UUIDs are required faster than 1us (i.e. > 1M UUIDs/sec), an internal
counter simulates 100ns intervals, which is the UUID resolution limit.
Each UUID within the same 1us tick will thus appear to be at successive
100ns intervals, which probably isn't true.

Should the internal counter exceed 9 within the same 1us period, it is
reset to 0 and the UUID sequence field is incremented, thus guaranteeing
uniqueness even though the time sequence will repeat.

This scheme raises the limit of time-based uniqueness to over 16G
UUIDs/sec, which is well beyond the best benchmarks seen to date.
(~14M/sec for v6 binary UUIDs on commodity hardware.)

Starting in version 0.38, the internal counter is moved to shared
memory, ensuring uniqueness among time-based UUIDs in forked or threaded
environments. Note that UUID must be loaded before forking or creating
threads for this guarantee to hold.
...
This software is Copyright (c) 2014-2026 by Rick Myers.
